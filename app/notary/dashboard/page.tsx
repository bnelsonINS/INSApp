...
function statusDot(status: string | null) {
  ...
  if (normalized === "not confirmed") return "bg-amber-500";
  if (normalized === "confirmed") return "bg-blue-500";
  if (normalized === "in progress") return "bg-purple-500";
  if (normalized === "late") return "bg-red-500";
  if (normalized === "signing complete") return "bg-orange-500";
...
export default async function NotaryDashboardPage() {
  ...
  return (
    <main className="space-y-6 p-4 sm:p-6">
      <section className="overflow-hidden rounded-2xl bg-slate-950 text-white shadow">
          ...
          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
            <div>
              <h2
                className={`text-lg font-bold ${
                  hasMissingReadinessItems ? "text-amber-950" : "text-blue-950"
                }`}
              >
                {hasMissingReadinessItems
              ...
              <p
                className={`mt-1 max-w-3xl text-sm ${
                  hasMissingReadinessItems ? "text-amber-800" : "text-blue-800"
                }`}
              >
                {hasMissingReadinessItems
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              {missingProfileItems.length > 0 && (
                <a
                  href="/notary/profile"
                  className="rounded-xl bg-amber-600 px-4 py-2 text-center text-sm font-bold text-white hover:bg-amber-700"
                >
                  Complete Profile
                </a>
              )}
              ...
              {missingCredentialItems.length > 0 && (
                <a
                  ...
                  className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-center text-sm font-bold text-slate-800 hover:bg-slate-50"
                >
                  Upload Credentials
                </a>
              )}
              ...
              {!hasMissingReadinessItems && (
                <a
                  href="/notary/credentials"
                  className="rounded-xl border border-blue-300 bg-white px-4 py-2 text-center text-sm font-bold text-blue-800 hover:bg-blue-50"
                >
                  Review Credentials
                </a>
              )}
            ...
            <div>
              ...
              <div className="mt-2 space-y-1">
                {credentialAlerts.slice(0, 2).map((alert) => (
                  <p key={alert} className="text-sm font-medium text-amber-800">
                    ⚠ {alert}
                  </p>
                ))}
              </div>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <a
          ...
          className="rounded-2xl border border-blue-100 bg-blue-50 p-5 shadow-sm hover:bg-blue-100"
        >
          ...
          <p className="mt-2 text-xs text-blue-700">Waiting on your response</p>
        </a>

        <a
          href="/notary/assignments"
          className="rounded-2xl border border-purple-100 bg-purple-50 p-5 shadow-sm hover:bg-purple-100"
        >
          ...
          <p className="mt-2 text-xs text-purple-700">Scheduled for today</p>
        </a>

        <a
          href="/notary/earnings"
          className="rounded-2xl border border-green-100 bg-green-50 p-5 shadow-sm hover:bg-green-100"
        >
        ...
        >
          <p
            className={`text-sm font-semibold ${
              credentialAlerts.length ? "text-amber-700" : "text-slate-600"
            }`}
          >
            Credential Alerts
          </p>
          <p
            className={`mt-2 text-4xl font-bold ${
              credentialAlerts.length ? "text-amber-950" : "text-slate-950"
            }`}
          >
            {credentialAlerts.length}
          </p>
          <p
            className={`mt-2 text-xs ${
              credentialAlerts.length ? "text-amber-700" : "text-slate-500"
            }`}
          >
                ...
                >
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex gap-4">
                      <div className="min-w-20 rounded-xl bg-slate-100 p-3 text-center">
          ...
          <section className="rounded-2xl bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-3">
              ...
              >
                ...
                <span className="font-bold">{notConfirmed}</span>
              </a>
              ...
              >
                ...
                <span className="font-bold">{signingComplete}</span>
              </a>
              ...
              >
                ...
                <span className="font-bold">{closedThisMonth.length}</span>
              </a>
            </div>
          </section>
        </aside>
        ...
        <section className="rounded-2xl bg-white p-5 shadow-sm">
          <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
            ...
            <a
              ...
              className="rounded-xl border px-4 py-2 text-center text-sm font-bold hover:bg-slate-50"
            >
              Manage Coverage
            </a>
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            <div className="rounded-xl bg-slate-50 p-4">
              ...
              <p className="mt-1 text-3xl font-bold">{countyCount ?? 0}</p>
            </div>

            <div className="rounded-xl bg-slate-50 p-4">
              <p className="text-sm font-semibold text-slate-500">ZIP Codes</p>
              <p className="mt-1 text-3xl font-bold">{zipCount ?? 0}</p>
            </div>
          </div>
        </section>
          ...
          <details className="mt-4 rounded-xl border p-4">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <div className="rounded-lg bg-amber-50 p-3">
                <p className="font-bold text-amber-900">Not Confirmed</p>
              ...
              <div className="rounded-lg bg-blue-50 p-3">
                <p className="font-bold text-blue-900">Confirmed</p>
              ...
              <div className="rounded-lg bg-purple-50 p-3">
                <p className="font-bold text-purple-900">In Progress</p>
              ...
              <div className="rounded-lg bg-orange-50 p-3">
                <p className="font-bold text-orange-900">Signing Complete</p>
              ...
              <div className="rounded-lg bg-green-50 p-3">
                <p className="font-bold text-green-900">Closed</p>
